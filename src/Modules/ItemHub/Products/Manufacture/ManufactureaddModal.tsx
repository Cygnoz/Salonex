...
import { manufactureData } from "../../../../Interface/ProductManufacture";
...
const ManufactureaddModal = ({onClose,editId}: Props) => {
  ...
  const {refreshContext,bmcrData}=useRegularApi()
   ...
   const setFormValues = (data: manufactureData) => {
     Object.keys(data).forEach((key) => {
       setValue(key as keyof manufactureData, data[key as keyof manufactureData]);
     });
   };
   ...
   useEffect(() => {
     if (editId) {
       ...
       if(bmcrData){
         ...
         const body = {
           ...bmcrData,
           name:bmcrData.manufacturerName
         }
   ...
   const onSubmit: SubmitHandler<manufactureData> = async (data) => {
     try {
       ...
       if (response && !error) {
         toast.success(`Manufacturer ${editId ? "updated" : "added"} successfully!`);
         console.log(response.data);
         onClose();
         const type={ type:"manufacturer" }
         refreshContext({bmcrType:type})
       } else {
         toast.error(error.response.data.message);
       }
     } catch (err) {
       console.error("Error submitting Manufacturer data:", err);
  ...
  return (
    <div className="p-4 bg-[#F8F4F4] rounded-lg">
       ...
       <h1 className="text-base font-bold text-deepStateBlue">{editId?'Edit':'Add'} Manufacture</h1>
